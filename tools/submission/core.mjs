...
export function auditToolDefinitions(tools, catalogue = {}) {
  ...
  const expectedTargetApprox = Number.isSafeInteger(catalogue.targetApprox) ? catalogue.targetApprox : null;
  ...
  return {
    ...
    expectedMinimum,
    expectedExact,
    expectedTargetApprox,
    targetDelta: expectedTargetApprox === null ? null : tools.length - expectedTargetApprox,
    names,
    duplicateNames,
    invalid,
...
export function reportMarkdown(report) {
  ...
  const blockers = report.blockingIssues.length
    ? report.blockingIssues.map((item) => `- ${escapeTable(item)}`).join('\n')
    : '- None.';
  const screenshots = report.screenshots.length
    ? report.screenshots.map((item) => `- \`${escapeTable(item)}\``).join('\n')
    : '- None generated.';
  const annotations = report.webmcp?.annotationAudit?.length
    ? report.webmcp.annotationAudit.filter((item) => item.risk !== 'LOW').map((item) =>
        `| ${escapeTable(item.tool)} | ${escapeTable(item.readOnlyHint)} | ${escapeTable(item.destructiveHint)} | ${escapeTable(item.idempotentHint)} | ${escapeTable(item.risk)} | ${escapeTable(item.recommendation)} |`
      ).join('\n') || '| — | — | — | — | — | No suspicious annotation. |'
    : '| — | — | — | — | — | Not measured. |';
  const responseSizes = report.webmcp?.responseSizes?.length
    ? report.webmcp.responseSizes.map((item) =>
        `| ${escapeTable(item.tool)} | ${escapeTable(item.scenario)} | ${item.characters} | ${item.bytes} | ${String(item.truncated)} | ${String(item.pageable)} | ${escapeTable(item.severity)} |`
      ).join('\n')
    : '| — | — | — | — | — | — | Not measured. |';
  const regressions = Object.entries(report.regressions ?? {}).map(([name, value]) =>
    `| ${escapeTable(name)} | ${escapeTable(value?.tests ?? '—')} | ${escapeTable(value?.passes ?? '—')} | ${escapeTable(value?.failures ?? '—')} | ${escapeTable(value?.status ?? '—')} |`
  ).join('\n') || '| — | — | — | — | — |';
  const hero = report.heroReliability;
  const heroText = hero
    ? `- Passes: ${hero.passes ?? 0} / ${hero.attempts ?? 0}\n- Not available: ${hero.notAvailable ?? 0}\n- Mean duration: ${hero.meanDurationMs ?? 'not available'} ms\n- p95 duration: ${hero.p95DurationMs ?? 'not available'} ms\n- Failure categories: \`${JSON.stringify(hero.failureCategories ?? {})}\``
    : '- Not measured.';
  const availability = report.futureAvailability
    ? Object.entries(report.futureAvailability).map(([key, value]) => `- ${key}: **${value ? 'available' : 'not available'}**`).join('\n')
    : '- Not measured.';
  return `# ROBO BRIDGE MCP Submission Gate\n\n` +
    ...
    `- Profile: ${report.mode?.profile ?? 'auto'}\n` +
    `- Browser: ${report.browser?.product ?? report.browser?.reason ?? 'not available'}\n` +
    `- WebMCP tools: ${report.webmcp?.toolCount ?? 'not available'}\n` +
    `- Native WebMCP acceptance claimed: **no**. Registration uses the production registrar with an injected capture interface.\n\n` +
    `## Result Summary\n\n` +
    ...
    `## Regression Results\n\n` +
    `| Suite | Tests or trials | Pass | Fail | Status |\n|---|---:|---:|---:|---|\n${regressions}\n\n` +
    `## WebMCP Annotation Audit\n\n` +
    `| Tool | Read only | Destructive | Idempotent | Risk | Recommendation |\n|---|---|---|---|---|---|\n${annotations}\n\n` +
    `## WebMCP Response Sizes\n\n` +
    `| Tool | Scenario | Characters | Bytes | Truncated | Pageable | Severity |\n|---|---|---:|---:|---|---|---|\n${responseSizes}\n\n` +
    `## Hero Reliability\n\n${heroText}\n\n` +
    `## Future Service Availability\n\n${availability}\n\n` +
    `## Console\n\n` +
    `- Blocking entries: ${report.console?.blocking?.length ?? 0}\n` +
    `- Errors: ${report.console?.errors?.length ?? 0}\n` +
    `- Warnings: ${report.console?.warnings?.length ?? 0}\n\n` +
    `## Evidence\n\n${screenshots}\n\n` +
    `## Rerun\n\n` +
    `- Complete gate: \`npm run submission:gate\`\n` +
    `- Smoke gate: \`npm run submission:smoke\`\n` +
    `- WebMCP audit: \`npm run webmcp:audit\`\n` +
    `- Reliability: \`npm run hero:3\` or \`npm run hero:10\`\n` +
    `- Final strict evidence: \`npm run release:evidence\`\n`;
}
