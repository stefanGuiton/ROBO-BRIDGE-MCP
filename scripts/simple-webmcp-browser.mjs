...
try {
  ...
  assert.equal(boot.names.length, 31);
  assert.equal(new Set(boot.names).size, 31);
  for (const name of ['request_more_bricks', 'get_scene_settings', 'update_scene_settings']) assert.ok(boot.names.includes(name));
