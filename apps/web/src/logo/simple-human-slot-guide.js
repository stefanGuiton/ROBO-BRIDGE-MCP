...
function mountPanel() {
  ...
  panel.setAttribute('aria-label', 'Guided Human contribution');
  panel.innerHTML = '<b>HUMAN CONTRIBUTION</b><span data-simple-human-slot>PLAN A STRUCTURE FIRST</span><small data-simple-human-pose></small><small data-simple-human-rule></small>';
  host.append(panel);
  return panel;
}
...
function refreshPanel() {
  ...
  if (!streamId) {
    panel.dataset.state = 'waiting';
    slot.textContent = 'PLAN A STRUCTURE FIRST';
    pose.textContent = `Example: ${FINAL_TOWER_REQUEST}`;
  ...
  if (adopted) {
    ...
    const actual = runtime.robotController.getBricks().find(brick => brick.id === adopted.actualBrickId);
    slot.textContent = `${adopted.placementId} · ${(actual?.colour ?? 'brick').toUpperCase()} ADOPTED`;
  ...
  const marker = runtime.renderer?.humanGuide;
  const guide = marker ? { ...marker, ...layerAndSlot(marker.placementId), targetPosition: marker.position,
    targetYawDeg: normaliseBrickYaw(marker.yawRad * 180 / Math.PI) } : selectHumanContributionGuide(status);
  if (!guide) {
    panel.dataset.state = 'waiting';
    slot.textContent = status.remainingPlacements === 0 ? 'BUILD COMPLETE' : 'WAIT FOR A READY SLOT';
  ...
  const heldId = runtime.humanBuildAdapter?.getState().heldBrickId;
  const held = heldId ? runtime.robotController.getBricks().find(brick => brick.id === heldId) : null;
  const colour = marker?.colour ?? held?.colour;
  const choice = colour ? `one ${colour} brick` : 'one compatible brick (for example a blue brick)';
  rule.textContent = `Aim at the amber marker: place ${choice} here. Release only on the normal VALID placement preview.`;
}
